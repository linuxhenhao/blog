...
## 总结
纵观CUBIC，KCP，BBR，CDG四种传输方式，CUBIC在丢包的情况下对带宽的利用率明显降低，即使这种丢
包不是由于拥塞导致的也会。KCP通过添加单独包的ACK，可以在重传时效率更高，这在丢包多的情况
下很有用，但是这需要server-client端共同安装，相对来说麻烦，如果不管拥塞情况保持发包速率，
这就有点不讲道理了：）。CDG则太绅士了，核心是通过斜率来判断拥塞情况，根据拥塞情况来决定
是否将丢包作为拥塞信号。但是由于CDG跟一大帮子不停的让带宽拥挤到丢包的CUBIC类的流量竞争时，
它还会在不发生丢包时概率性降低发送速率，那怎么玩？相对来说，BBR是最和谐的了，不管丢包，利用
间歇性的流量增长探测带宽瓶颈值大小，一直更新RTT，让整个连接的发包速率保持在靠近用满带宽，
而又填充路由缓存的理想位置附近。
